        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((product) => (
              <div
                key={product.id}
                className="card relative w-96 h-60 bg-black rounded-8 overflow-hidden"
              >
                <img
                  src={product.image}
                  alt={product.name}
                  className="card_image block w-full h-full object-cover object-center transition-transform duration-250"
                />
                <div className="card_body inset-0 z-[-1] flex items-end justify-between px-4 py-2 text-yellow-500">
                  <div>
                    ...
                    <p className="card_author">Price: ${product.price.toFixed(2)}</p>
                  </div>
